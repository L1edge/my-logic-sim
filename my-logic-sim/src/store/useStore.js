...
const useStore = create((set, get) => ({
  // ... (projects, activeProjectId залишаються без змін) ...
  projects: {
    [defaultProjectId]: { id: defaultProjectId, name: 'Untitled Circuit', nodes: [], edges: [] }
  },
  activeProjectId: defaultProjectId,

  // === ДОДАЄМО ЛІЧИЛЬНИК ДЛЯ ТЕСТБЕНЧА ===
  isRunning: false,
  intervalId: null,
  testBenchCounter: 0, // <--- НОВЕ: Зберігає поточний стан входів (0, 1, 2, 3...)

  theme: 'dark',
  setTheme: (theme) => set({ theme }),

  // ... (createNewProject, closeProject, setActiveProject, renameProject, loadGraph - БЕЗ ЗМІН) ...
  createNewProject: () => {
    const newId = nanoid();
    set(state => ({
  ...
  stopSimulation: () => {
    ...
    // Скидаємо лічильник тестбенча при зупинці
    set({ 
        isRunning: false, 
        intervalId: null,
        testBenchCounter: 0, // <--- СКИДАННЯ
        projects: { ...projects, [activeProjectId]: { ...currentProject, nodes: resetNodes, edges: resetEdges } }
    });
  },

  // === НОВИЙ STEP (TESTBENCH MODE) ===
  stepSimulation: () => {
    const { activeProjectId, projects, testBenchCounter } = get();
    const currentProject = projects[activeProjectId];
    
    // 1. Знаходимо всі входи і сортуємо їх (за Y, щоб було як на схемі зверху вниз)
    const inputNodes = currentProject.nodes
        .filter(n => n.type === 'inputNode')
        .sort((a, b) => a.position.y - b.position.y);

    if (inputNodes.length > 0) {
        // 2. Оновлюємо значення входів згідно з лічильником (Testbench Logic)
        const newNodes = currentProject.nodes.map(node => {
            if (node.type === 'inputNode') {
                // Знаходимо індекс цього інпута в відсортованому списку
                const index = inputNodes.findIndex(n => n.id === node.id);
                // Беремо потрібний біт з лічильника (Bitwise shift)
                // Наприклад, якщо counter = 5 (101), то input[0]=1, input[1]=0, input[2]=1
        ...
        // Зберігаємо оновлені ноди з новими значеннями
        set({
             projects: {
                ...projects,
                [activeProjectId]: { ...currentProject, nodes: newNodes }
             },
             testBenchCounter: testBenchCounter + 1 // Готуємось до наступного кроку
        });
    }

    // 3. Запускаємо звичайний розрахунок логіки
    get().runSimulationStep();
  },
